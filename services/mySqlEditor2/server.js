...
app.get('/api/schema', (req, res) => {
  ...
  loadSchema(conn)
    ...
    .catch(err => {
      res.status(500).json({ error: normalizeDbError(err, conn) });
    });
});
...
function buildMysqlConfig(conn) {
  return {
    ...
    password: conn.password || '',
    database: conn.database || undefined,
    // Use a higher timeout than mysql2 defaults to reduce false timeouts on slow links/VPNs.
